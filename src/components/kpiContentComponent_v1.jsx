        ...
        <div className="kpi-content">
            <div className="kpi-content-names">
                {props.items.map((item) => {
                    return <div key={item.name + item.percent + item.type + item.kpi} className="kpi-content-names__item">{item.name}</div>
                })}
            </div>
...
KpiContent_v1.propTypes = {
    items: PropTypes.array, 
    props: PropTypes.object
};
export default KpiContent_v1;